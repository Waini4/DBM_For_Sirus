...
DBM:RegisterMapSize("BlackTemple", -- Black Temple (Raid-BC). Has DungeonUsesTerrainMap()
	0, 783, 3333435,
	1, 522, 916626,
	2, 1252.2495784784999, 834.833007813,
	3, 975, 650,
	4, 1005, 670,
	5, 440.000976562, 293.333984375,
	6, 670, 446.66668701599986,
	7, 705, 470,
	8, 355, 236.66662597599998
)
...
DBM:RegisterMapSize("CoTStratholme",
	-- The Culling of Stratholme (Party-WotLK) ; API returns levels 1 and 2 - this is corrected with DungeonUsesTerrainMap()
...
DBM:RegisterMapSize("Karazhan", -- Karazhan (Raid-BC)
	...
	18, 550.04882811999983, 366.69921880000038,
    19, 257.85986329, 171.90625,
    20, 345.1494140599998, 230.09960940000019,
    21, 520.04882811999983, 346.69921880000038,
    22, 234.14990233999993, 156.09960940000019,
    23, 581.54882811999983, 387.69921880000038,
    24, 191.54882811999983, 127.69921880000038,
    25, 139.35058593999997, 92.90039059999981,
    26, 760.04882811999983, 506.69921880000038,
    27, 450.25, 300.16601559999981,
    28, 271.05004882999992, 180.69921880000038,
    29, 595.04882811999983, 396.69921880000038,
    30, 529.04882812, 352.69921880000038,
    31, 245.25, 163.5,
    32, 211.14990233999993, 140.765625,
    33, 101.25, 67.5,
    34, 341.24999999999977, 227.5
)
...
function createFrame()
	...
	frame:SetPoint(DBM.Options.RangeFramePoint, UIParent, DBM.Options.RangeFramePoint, DBM.Options.RangeFrameX,
...
function createRadarFrame()
	...
	radarFrame:SetPoint(DBM.Options.RangeFrameRadarPoint, UIParent, DBM.Options.RangeFrameRadarPoint,
		DBM.Options.RangeFrameRadarX, DBM.Options.RangeFrameRadarY)
...
do
	...
	function getDistanceBetween(uId, x, y)
		...
		if type(x) == "string" and UnitExists(x) then -- arguments: uId, uId2
			...
			if not x then
				print("getDistanceBetween failed for: " ..
					uId .. " (" .. tostring(UnitExists(uId)) .. ") and " .. uId2 .. " (" .. tostring(UnitExists(uId2)) .. ")")
				return
			end
		end
